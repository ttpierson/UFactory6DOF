...
def main():
    ...
    #Sweep
    #This is my sweep fuction, mess around with quaternation if needed. Rotates on xy plane or about z axis
    ...
    #Full azemuth and theta values, 

    L = 0.3

    for phi in range(-50, 50, 10):
        node.get_logger().info(f"Phi: {phi} degrees")

        for angle in range(-50, 50, 10):
            radT = math.radians(angle)
            radP = math.radians(phi)

            x3 = 0.3 + L - L*math.cos(radT)*math.cos(radP)
            y3 = L*math.sin(radT)*math.cos(radP)
            z3 = 0.3+L*math.sin(radP)
            q3 = quaternion_from_euler(math.pi/2+radP, 0.0, -radT+math.pi/2)

            moveit2.move_to_pose(
                position=Point(x=x3, y=y3, z=z3),
                quat_xyzw=[q3[0], q3[1], q3[2], q3[3]],
            )
            moveit2.wait_until_executed()
            time.sleep(0.5)  
            node.get_logger().info(f"Pose at theta {angle} degrees: x={x3:.3f}, y={y3:.3f}, z={z3:.3f}")
